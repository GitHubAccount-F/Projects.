...
class BinaryLogReg:
    ...
    def mu(self, X: np.ndarray, y: np.ndarray) -> np.ndarray:
        ...
        """
        inner = self.bias + np.matmul(X, self.weight)
        outer = 1 / (1 + np.exp(-y * inner))
        return outer
    ...
    def loss(self, X: np.ndarray, y: np.ndarray) -> float:
        ...
        """
        log_inner = np.log(1 + np.exp( -1 * (X @ self.weight * y)  - (y * self.bias)))
        
        regularization = self._lambda * np.linalg.norm(self.weight, ord = 2) ** 2
        return (np.sum(log_inner) / len(y)) + regularization
    ...
    def gradient_J_weight(self, X: np.ndarray, y: np.ndarray) -> np.ndarray:
        ...
        """
        mu = self.mu(X, y)
        regularization = self.weight * 2 * self._lambda
        first = X * (y[:, np.newaxis] * -1)
        second = (1 - mu)
        third = np.matmul(np.transpose(first), second)
        gradient = (third / len(X)) + regularization
        return gradient
    ...
    def predict(self, X: np.ndarray) -> np.ndarray:
        ...
        """
        initialGuess = (X @ self.weight) + self.bias

        # Transform using sigmoid
        new_guess = 1 / (1 + np.exp((-1) * initialGuess))
        new_guess = np.piecewise(new_guess, [new_guess > 0.5, new_guess <= 0.5], [1, -1])
        return new_guess
    ...
    def train(
        ...
        """
        #batch_size = len(X_train) # for gradient descent
        ...
        # 1 data for each epoch
        # mini-batch: for each epoch, shuffle X-train, 
        for i in range(epochs):
            for j in range(num_batches):
                rand = RNG.randint(len(X_train))
                self.step(X_train[rand], np.array([y_train[rand]]), learning_rate = learning_rate)
        ...
        '''
        for mini-batch
        all_data = np.append(X_train, y_train[:, np.newaxis], axis = 1)
        for i in range(epochs):
            print(i)
            for j in range(num_batches):
                # shuffle x
                ...
                last_column = all_data[:, -1]
                self.step(all_data[:batch_size,:-1], last_column[:batch_size], learning_rate = learning_rate)
            #print(self.loss(X_train, y_train))
            #print(self.loss(X_test, y_test))
            result["train_losses"].append(self.loss(X_train, y_train))
            result["train_errors"].append(self.misclassification_error(X_train, y_train))
            result["test_losses"].append(self.loss(X_test, y_test))
            result["test_errors"].append(self.misclassification_error(X_test, y_test))
