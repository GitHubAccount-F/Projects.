...
def train(x: np.ndarray, y: np.ndarray, _lambda: float) -> np.ndarray:
    ...
    """
    # Formula used: (X^T*X + lambda*I_N) W = X^T*Y
    # W should be d x k
    # np.shape[1] gives number of columns in x
    colSize = len(x[0])
    w = sc.linalg.solve(np.add(np.matmul(np.transpose(x),x),np.identity(colSize)*_lambda),np.matmul(np.transpose(x),y))
    return w
...
def predict(x: np.ndarray, w: np.ndarray) -> np.ndarray:
    ...
    """
    product = x.dot(w)
    prediction = product.argmax(axis = 1)
    return prediction
...
def one_hot(y: np.ndarray, num_classes: int) -> np.ndarray:
    ...
    Example:
        ...
        ```
    """
    zeroMatrix = np.zeros((len(y), num_classes))
