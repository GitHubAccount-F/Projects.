...
def train(
    ...
    loss_func = criterion()
    print(model)
    optimizer = optimizer(model.parameters(), 0.01)
    best_loss = float("inf")
    result = {'train':[], 'val':[]}
    for i in range(epochs):
        ...
        result["train"].append(total / len(train_loader))
        ...
        if val_loader is not None:
            with torch.no_grad():
                ...
                for a, b in val_loader: 
                    output_val = model(a)
                    loss_val = val_loss(output_val, b)
                    total_val += loss_val.item()
                result["val"].append(total_val/len(val_loader))
        # save model when loss is minimum
        if (total_val/len(val_loader) < best_loss):
            best_loss = total_val/len(val_loader)
