...
class LinearLayer(nn.Module):
    ...
    def __init__(
        ...
        self.weight = nn.Parameter(
            data=torch.randn((dim_in, dim_out), generator=generator, dtype=torch.float),
            requires_grad=True,
        )

        self.bias = nn.Parameter(
            data=torch.randn((dim_out), generator=generator, dtype=torch.float),
            requires_grad=True,
        )
